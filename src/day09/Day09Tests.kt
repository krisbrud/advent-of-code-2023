...
class Day09Tests {
    ...
    @Test
    fun `should predict backwards correctly`() {
        assertEquals(-3, history1.diffUntilAllZeroes().predictBackwards())
        assertEquals(0, history2.diffUntilAllZeroes().predictBackwards())
        assertEquals(5, history3.diffUntilAllZeroes().predictBackwards())
    }
}